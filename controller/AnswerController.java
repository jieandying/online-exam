...
public class AnswerController {
    ...
    @GetMapping("/answers/{page}/{size}/{subject}/{section}/{question}/{type}/{level}")
    public ApiResult findAllQuestion(
            ...
            @PathVariable("type") String type, @PathVariable("level") String level) {
        Page<AnswerVO> answerVOPage = new Page<>(page, size);
        IPage<AnswerVO> answerVOIPage = answerService.findAll(answerVOPage, subject, section, question, type, level);
