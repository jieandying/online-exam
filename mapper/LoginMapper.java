...
public interface LoginMapper {
    ...
    // 通过手机号查询用户（短信登录）
    @Select("select adminId,adminName,sex,tel,email,cardId,role from `admin` where tel = #{phone}")
    public Admin findAdminByPhone(@Param("phone") String phone);

    @Select("select teacherId,teacherName,institute,sex,tel,email,cardId,type,role from teacher where tel = #{phone}")
    public Teacher findTeacherByPhone(@Param("phone") String phone);

    @Select("select studentId,studentName,grade,major,clazz,institute,tel,email,cardId,sex,role from student where tel = #{phone}")
    public Student findStudentByPhone(@Param("phone") String phone);
}
